...
IClaireonTool::FToolResult ClaireonTool_DataTableExportJson::Execute(const TSharedPtr<FJsonObject>& Arguments)
{
	FString AssetPath;
	...
	UE_LOG(LogClaireon, Display, TEXT("[MCP] editor.datatable.export_json: asset_path=%s"), *AssetPath);
	...
	FString JsonOutput = Table->GetTableAsJSON(EDataTableExportFlags::None);
	if (JsonOutput.StartsWith(TEXT("Missing RowStruct")))
	{
		return MakeErrorResult(FString::Printf(TEXT("Table has no row struct: %s"), *AssetPath));
	}

	return MakeSuccessResult(nullptr, JsonOutput);
}
